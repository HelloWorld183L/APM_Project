...
```{r}
ggplot(data=posts_df, aes(x=is_bullying)) + geom_bar()
```
...
```{r}
posts_df_sample <- ovun.sample(is_bullying~., data=posts_df, method="under")$data

ggplot(data=posts_df_sample, aes(x=is_bullying)) + geom_bar()
```
This data should be balanced now. However, this comes at the expense of having much less data to work with.

```{r}
write.csv(posts_df_sample, file="Data/clean_posts_balanced_sample.csv")
```
## Step 4: EDA
