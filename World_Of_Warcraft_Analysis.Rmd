...
lol_posts_df <- read.csv("Data/posts_wow.csv")
...
colnames(merged_df)[colnames(merged_df) == 'post_number.x'] <- 'post_number'
colnames(merged_df)[colnames(merged_df) == 'topic_id.x'] <- 'topic_id'
colnames(merged_df)[colnames(merged_df) == 'dataset.x'] <- 'dataset'
colnames(merged_df)[colnames(merged_df) == 'id.x'] <- 'id'
...
```{r}
posts_df$html_message <- mapply(remove_html, posts_df$html_message)
```
...
```{r}
posts_df <- posts_df[!apply(posts_df, 1, function(msg) any(msg=="")),]
```
