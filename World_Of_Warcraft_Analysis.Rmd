...
glm_tuned_pred_y = predict(rf_model, test_balanced)
```
...
glm_confusion_matrix <- caret::confusionMatrix(data=glmnet_pred_y, reference=test_balanced$is_bullying, mode="everything")
```

```{r}
svm_linear_tuned_cf_matrix <- caret::confusionMatrix(data=svm_tuned_pred_y, reference=test_balanced$is_bullying, mode="everything")
svm_poly_cf_matrix <- caret::confusionMatrix(data=svm_poly_pred_y, reference=test_balanced$is_bullying, mode="everything")
svm_rbf_cf_matrix <- caret::confusionMatrix(data=svm_rbf_pred_y, reference=test_balanced$is_bullying, mode="everything")

mlp_tuned_cf_matrix <- caret::confusionMatrix(data=mlp_tuned_pred_y, reference=test_balanced$is_bullying, mode="everything")
glm_tuned_cf_matrix <- caret::confusionMatrix(data=glm_tuned_pred_y, reference=test_balanced$is_bullying, mode="everything")
```
...
converted_pred_y_glmnet <- as.numeric(levels(rf_pred_y))[glmnet_pred_y]
...
glmnet_roc <- roc(test_balanced$is_bullying, converted_pred_y_glmnet, plot=TRUE, print.auc=TRUE, print.auc.y=0.6, col="green", lwd=4, legacy.axes=TRUE, add=TRUE)

legend("bottomright", legend=c("SVM", "MLP", "GLMNET"), col=c("red", "blue", "green"), lwd=4)
```

#### Comparing tuned SVM models
```{r}
converted_pred_y_svm_linear_tuned <- as.numeric(levels(svm_tuned_pred_y))[svm_tuned_pred_y]
converted_pred_y_svm_poly_tuned<- as.numeric(levels(svm_poly_pred_y))[svm_poly_pred_y]
converted_pred_y_svm_rbf_tuned <- as.numeric(levels(svm_rbf_pred_y))[svm_rbf_pred_y]

par(pty="s")
svm_linear_roc <- roc(test_balanced$is_bullying~converted_pred_y_svm_linear_tuned, plot=TRUE, print.auc=TRUE, col="red", lwd=4, legacy.axes=TRUE, main="ROC Curves")
svm_poly_roc <- roc(test_balanced$is_bullying~converted_pred_y_svm_poly_tuned, plot=TRUE, print.auc=TRUE, print.auc.y=0.4, col="blue", lwd=4, legacy.axes=TRUE, add=TRUE)
svm_rbf_roc <- roc(test_balanced$is_bullying, converted_pred_y_svm_rbf_tuned, plot=TRUE, print.auc=TRUE, print.auc.y=0.6, col="green", lwd=4, legacy.axes=TRUE, add=TRUE)

legend("bottomright", legend=c("SVM Linear", "SVM Poly", "SVM RBF"), col=c("red", "blue", "green"), lwd=4)
```
...
#### Tuned models
```{r}
converted_pred_y_mlp_tuned <- as.numeric(levels(mlp_tuned_pred_y))[mlp_tuned_pred_y]
converted_pred_y_glmnet_tuned <- as.numeric(levels(glm_tuned_pred_y))[glm_tuned_pred_y]

par(pty="s")
svm_poly_roc <- roc(test_balanced$is_bullying~converted_pred_y_svm_poly_tuned, plot=TRUE, print.auc=TRUE, col="red", lwd=4, legacy.axes=TRUE, main="ROC Curves")
mlp_tuned_roc <- roc(test_balanced$is_bullying~converted_pred_y_mlp, plot=TRUE, print.auc=TRUE, print.auc.y=0.4, col="blue", lwd=4, legacy.axes=TRUE, add=TRUE)
glmnet_tuned_roc <- roc(test_balanced$is_bullying, converted_pred_y_rf, plot=TRUE, print.auc=TRUE, print.auc.y=0.6, col="green", lwd=4, legacy.axes=TRUE, add=TRUE)

legend("bottomright", legend=c("SVM Poly", "MLP Tuned", "GLMNET"), col=c("red", "blue", "green"), lwd=4)
```
...
```{r}
svm_precision <- precision(svm_tuned_pred_y, test_balanced$is_bullying)
svm_sensitivity <- sensitivity(svm_tuned_pred_y, test_balanced$is_bullying)
svm_f1_score <- F_meas(svm_tuned_pred_y, test_balanced$is_bullying)
svm_auc <- auc(svm_roc)

svm_poly_precision <- precision(svm_poly_pred_y, test_balanced$is_bullying)
svm_poly_sensitivity <- sensitivity(svm_poly_pred_y, test_balanced$is_bullying)
svm_poly_f1_score <- F_meas(svm_poly_pred_y, test_balanced$is_bullying)
svm_poly_auc <- auc(svm_poly_roc)

svm_rbf_precision <- precision(svm_rbf_pred_y, test_balanced$is_bullying)
svm_rbf_sensitivity <- sensitivity(svm_rbf_pred_y, test_balanced$is_bullying)
svm_rbf_f1_score <- F_meas(svm_rbf_pred_y, test_balanced$is_bullying)
svm_rbf_auc <- auc(svm_rbf_roc)
```
...
mlp_tuned_precision <- precision(mlp_tuned_pred_y, test_balanced$is_bullying)
mlp_tuned_sensitivity <- sensitivity(mlp_tuned_pred_y, test_balanced$is_bullying)
mlp_tuned_f1_score <- F_meas(mlp_tuned_pred_y, test_balanced$is_bullying)
mlp_tuned_auc <- auc(mlp_tuned_roc)
```

```{r}
glmnet_precision <- precision(glm_tuned_pred_y, test_balanced$is_bullying)
glmnet_sensitivity <- sensitivity(glm_tuned_pred_y, test_balanced$is_bullying)
glmnet_f1_score <- F_meas(glm_tuned_pred_y, test_balanced$is_bullying)
glmnet_auc <- auc(glmnet_tuned_roc)
```

#### Table
```{r}
Model_Name <- c("SVM Linear", "SVM Poly", "SVM RBF", "MLP", "GLMNET")
Precision <- c(svm_precision, svm_poly_precision, svm_rbf_precision, mlp_precision, glmnet_precision)
Sensitivity <- c(svm_sensitivity, svm_poly_sensitivity, svm_rbf_sensitivity, mlp_sensitivity, glmnet_sensitivity)
F1_Score <- c(svm_f1_score, svm_poly_f1_score, svm_rbf_f1_score, mlp_f1_score, glmnet_f1_score)
AUC <- c(svm_auc, svm_poly_auc, svm_rbf_auc, mlp_auc, glmnet_auc)
Time_to_Train_secs <- c(svm_time_taken, svm_poly_tuned_time_taken, svm_rbf_tuned_time_taken, mlp_time_taken, glmnet_time_taken)
