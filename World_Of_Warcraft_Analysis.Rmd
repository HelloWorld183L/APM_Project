...
# - Correct popular abbrevations and acronyms (e.g. WoD, lol)
clean_data <- function(html_msg) {
  html_msg <- gsub("<.*?>", "", html_msg)
  html_msg <- gsub('[[:punct:] ]+', ' ', html_msg)
  html_msg <- gsub('http\\S+\\s*', '', html_msg)
  return(trimws(html_msg))
}
```

```{r}
wow_posts_df$html_message <- clean_data(wow_posts_df$html_message)
```
