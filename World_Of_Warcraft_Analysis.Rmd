...
```{r}
nrow(bullying_wow) / nrow(wow_posts_df) * 100
```
...
svm_linear_tuned = caret::train(is_bullying~ ., data = train_df, method = "svmLinear", trControl = train_control, tuneGrid = svm_linear_grid)
...
# training a svmLinear classification model while tuning parameters
svm_model_poly = caret::train(is_bullying~ ., data = train_df, method = "svmPoly", trControl = train_control, tuneGrid = svm_poly_grid)
...
# training a svmLinear classification model while tuning parameters
svm_model_rbf = caret::train(is_bullying~ ., data = train_df, method = "svmRadial", trControl = train_control, tuneGrid = svm_rbf_grid)
